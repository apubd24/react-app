...
const DeviceManagment = () => {
  const { devices, isLoading, handleDeleteDevice, handleToggleStatus } = useDeviceList();
  ...
  const {
    ...
    indexOfFirstRow,
    indexOfLastRow,
  } = useDevicePagination(filteredDevices, [
    searchTerm,
    filterStatus,
    customerFilter,
    vendorFilter,
  ...
  const { downloadCSV, downloadExcel, downloadPDF } = useDeviceExport();

  return (
    <div className="space-y-6 w-full max-w-none px-6">
      {/* HEADER SECTION */}
      <DeviceTableHeader
        onDownloadCSV={() => downloadCSV(filteredDevices)}
        onDownloadExcel={() => downloadExcel(filteredDevices)}
        onDownloadPDF={() => downloadPDF(filteredDevices)}
      />
      ...
      <DeviceFilters
        ...
        onAssetStatusChange={setAssetStatusFilter}
...
// Single device management page with all features integrated into one component for simplicity. In a real app, you might want to split this into multiple components and use context or a state management library for better organization.
